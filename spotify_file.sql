...
select count(billing_country) as ct,billing_country 
from invoice
...
select customer.first_name,customer.last_name,sum(invoice.total) as total from customer
left join invoice
...
select count(album.album_id) as number_of_songs,artist.name from artist
...
select track.name,track.milliseconds as time
from track
where track.milliseconds > (select avg(milliseconds) as average_time from track)
order by time desc;
